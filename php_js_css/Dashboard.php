...
if(!isset($_SESSION['role']) || $_SESSION['role'] !== 'admin'){
    echo "<h2>Access Denied</h2><p>Ju nuk keni të drejta për këtë faqe.</p>";
    exit;
}
...
if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    if($action=='create'){
        ...
        if(empty($name)||empty($email)||empty($password)||empty($confirm)){
            ...
            if($userModel->register($name,$email,$password)) $success="User u krijua me sukses!";
    ...
    if($action=='update'){
        ...
        if($userModel->updateUser($id,$name,$email,$password,$role)) $success="User u përditësua me sukses!";
        else $error="Gabim gjatë përditësimit!";
    }
}

if(isset($_GET['delete'])){
    $id=$_GET['delete'];
    if($userModel->deleteUser($id)) $success="User u fshi me sukses!";
...
if(isset($_GET['delete_msg'])){
    ...
    if($stmt->execute([':id'=>$id])){
        $success = "Mesazhi u fshi me sukses!";
